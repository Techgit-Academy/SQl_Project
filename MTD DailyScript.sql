SELECT CAST(transaction_date AS DATE) Transact_date, product_category, tenant_name,payment_channel,
generation, product_code,b.region,
...
FROM `daily_transactions_curr_MTD`a
LEFT JOIN
(
SELECT region, contract
FROM customer_location) b
ON a.contract= b.contract
WHERE repayment_status = 'Subsequent_Repayment' #and product_category NOT in ('Initial', 'Outright')
GROUP BY  product_category,Revenue_Category,Transact_date,tenant_name,payment_channel,b.contract,
generation, product_code, tenant_name,b.region;
...
SELECT * FROM `daily_transactions` WHERE report_date = '2023-07-01'




SELECT CAST(transaction_date AS DATE) Transact_date,contract, contract_num, product_category,
CASE WHEN tenant_name = 'MTN_NG' AND Product_Type = 'Classic' THEN 'MTN-Legacy'
WHEN tenant_name = 'MTN_NG' AND Product_Type <> 'Classic' THEN 'MTN-Unified'
...
FROM 
`daily_transactions`
WHERE repayment_status = 'Subsequent_Repayment' AND report_date = '2023-06-01'
GROUP BY Revenue_Category,Transact_date, contract, contract_num, product_category;
